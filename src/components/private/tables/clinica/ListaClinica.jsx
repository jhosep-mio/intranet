...
const ListaClinica = () => {
  ...
  const preguntar = (id) => {
    Swal.fire({
      title: `¿Estás seguro de eliminar la clínica N° ${id}?`,
      showDenyButton: true,
  ...
  const deleteClinica = async (id) => {
    try {
      ...
      if (error.request.response.includes("Integrity constraint violation")) {
        Swal.fire("La clínica esta siendo usada en algún doctor", "", "error");
      } else {
        Swal.fire("Error al eliminar el registro", "", "error");
      }
    }
                        ...
                        {clinica.id != 0 && (
                          <td className="text-center">
                            <Link
                              ...
                              to={`/admin/clinicas/editar/${clinica.id}`}
                            >
